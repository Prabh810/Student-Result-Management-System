...
class Register:
    def __init__(self, root):
        ...
        #===============Terms 
        self.var_chk = IntVar()
        chk = Checkbutton(frame1, text="I Agree The Terms & Conditions",font=("times new roman", 12), bg="white", variable=self.var_chk, onvalue=1, offvalue=0).place(x=50,y=380)

        #===============Button
        self.btn_img = ImageTk.PhotoImage(file="images//register.png")
        btn_register= Button(frame1, image=self.btn_img, bd=0, cursor="hand2", command=self.register).place(x=50,y=420)
...
#=======================================================
    ...
    def register(self):
        if self.txt_fname.get() == "" or self.txt_email.get()== "" or self.txt_contact.get()=="" or self.comb_quest.get()=="Select" or self.txt_answer.get()=="" or self.txt_password.get()=="" or self.txt_cpassword.get()=="":
            ...
            try:
                ...
                messagebox.showerror("Error", f"Erro due to{str(es)}", parent=self.root)
